				...
				try {
					...
					if(rsJ.next()) {
						...
						try {
				    		...
				    		pst.setInt(2, IDgame);
				    		pst.executeUpdate();
				    		JOptionPane.showMessageDialog(null, "Gracias por comprar ''" + juego.getNombre() + "'', ahora podras verlo en tu librer�a.");
						}catch (Exception x) {
    	...
    	try {
    		...
    		if(rs.next()) {
    			...
    			try {
    				...
    				PreparedStatement pst=reg.prepareStatement(sql);
    	    		pst.executeUpdate();
    	    		JOptionPane.showMessageDialog(null, "Se ha eliminado ''" + juego.getNombre() + "'' de tu librer�a.");
    			}catch (Exception x) {
    				JOptionPane.showMessageDialog(null, "ERROR: " + x);
    			}
    		}
