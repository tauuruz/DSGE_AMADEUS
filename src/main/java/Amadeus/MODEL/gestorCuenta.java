...
public class gestorCuenta {
	...
	public static void cambiarPssw(int id_usuario,String newPass) {
		...
		if(stringLength<8) {
			JOptionPane.showMessageDialog(null, "La longitud de la contrase�a debe ser minimo de 8 car�cteres.");
		}
		else {
			con = new DB_Connector();
	        Connection reg = con.getConnection();
	        ...
	        try {
	        	...
	        	pst.executeUpdate();
	        	JOptionPane.showMessageDialog(null, "La contrase�a se actualiz� correctamente.");
			}catch (Exception x) {
				JOptionPane.showMessageDialog(null, "ERROR: " + x);
			}
		}
	...
	public static void registrarUsuario(String usuario,String nombre,String paterno,String materno,String direccion,String telefono,String correo, String password, boolean premium) {
        ...
        try {
			...
			if(rsE.next()) {
				...
				if(rs.next()) {
					JOptionPane.showMessageDialog(null, "El usuario ''" + usuario + "'' no est� disponible, por favor elija otro.");
				}
				else {
